...
- **Disabled:** the icon renders as the **scissors with a horizontal line drawn through the centre**, crossing the pivot point. This is perpendicular to the icon's axis of symmetry, so it reads clearly without merging into either blade. The line is composited over the base symbol at render time using `NSImage`'s drawing API and the result is still a template image, so it adapts to light/dark mode just like the enabled state. No custom image assets are needed.
